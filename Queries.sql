...
select inst_name, median_earnings_10_years_after, mean_earnings_10_years_after,
median_earnings_10_years_after - median_earnings_6_years_after as earnings_growth,
mean_earnings_10_years_after - median_earnings_10_years_after as med_mean_diff
from earnings as e join costs as c on
e.unit_id = c.unit_id join universities as u on e.unit_id = u.unit_id 
order by med_mean_diff desc;
...
from earnings as e join costs as c on
e.unit_id = c.unit_id join universities as u on e.unit_id = u.unit_id 
where mean_earnings_10_years_after is not null order by med_mean_diff;
-- a negative difference would mean there are some people that make very little afterwards
...
select broad_subject_area, max(cip_2_digit) as cip, -- wrap w/ agg function to allow us to view it
	avg(median_earnings_2_years_after) as avg_earnings,
...
left join universities as u on u.unit_id = d.unit_id -- left join to include the nulls (see below)†
where cip_2_digit = 61 -- broad_subject_area like "%military%" -- 
order by median_earnings_2_years_after desc;
-- 61 seems to be entirely radiology
-- note how the majority are null (since most were privacy suppressed)
...
select broad_subject_area, max(cip_2_digit) as cip, avg(median_earnings_2_years_after) as avg_earnings,
	avg(number_working_2_years_after / (number_working_2_years_after + number_not_working_2_years_after)) as avg_percent_working,
...
select * from universities where unit_id = 105215; -- this is the school with the fewest female percentage
select * from broad_cip_areas;
